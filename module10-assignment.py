...
df_typed['age'] = df_typed['age'].fillna(df_typed['age'].median())
...
# - Low: < 5 purchases
def categorize_purchase_frequency(purchases):
    if purchases >= 10:
        return "High"
    elif purchases >= 5:
        return "Medium"
...
avg_spent_by_loyalty = (
    df_final[df_final['loyalty_status'] != 'Unknown']
    .groupby('loyalty_status')['total_spent']
    .mean()
)
...
print("- Data Type Issues: mixed date formats, currency symbols/commas in total_spent, inconsistent numeric formats")
...
print(f"- Phone Numbers: Converted to {phone_format} format")
...
print(f"- Satisfaction vs Spending Correlation: {satisfaction_spend_corr:.3f}")
